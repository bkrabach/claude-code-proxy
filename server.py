...
AZURE_USE_MANAGED_IDENTITY = os.environ.get("AZURE_USE_MANAGED_IDENTITY", "false").lower() == "true"

def get_managed_identity_token() -> Optional[str]:
    """Acquire an Azure AD token for the OpenAI resource."""
    try:
        from azure.identity import DefaultAzureCredential
    except Exception as e:
        logger.error(f"azure.identity not installed: {e}")
        return None
    ...
    try:
        ...
        logger.error(f"Failed to acquire managed identity token: {exc}")
        return None
    ...
    def validate_model_field(cls, v, info): # Renamed to avoid conflict
        ...
        # Add prefixes to non-mapped models if they match known lists
            ...
            if clean_v in GEMINI_MODELS and not v.startswith('gemini/'):
                ...
                mapped = True # Technically mapped to add prefix
            elif clean_v not in GEMINI_MODELS and not v.startswith('azure/') and PREFERRED_PROVIDER == 'azure':
...
def convert_anthropic_to_litellm(anthropic_request: MessagesRequest) -> Dict[str, Any]:
    ...
    if (
        ...
        logger.debug(
            f"Capping max_tokens to 16384 for OpenAI/Gemini/Azure model (original value: {anthropic_request.max_tokens})"
        )
...
async def create_message(
    ...
    try:
        ...
        if request.model.startswith("openai/"):
            ...
            elif AZURE_USE_MANAGED_IDENTITY:
                token = get_managed_identity_token()
                if token:
                    litellm_request["azure_ad_token"] = token
